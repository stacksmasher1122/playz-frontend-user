...
import 'package:redesign/view/USER/Home/Scoreboard/Karate/widgets/karate_ka_card.dart';

class KarateSetupScreen extends StatelessWidget {
  const KarateSetupScreen({super.key});

  @override
  Widget build(BuildContext context) {
    ResponsiveHelper.init(context);
    final controller = Get.put(KarateController());
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              // 1. Category Selector (Senior 3 Min vs Junior 2 Min)
              ...
              // AKA (Red Belt / Corner)
              KarateKaCard(
                cornerTitle: 'AKA (Red Belt)',
                nameController: controller.akaFighterController,
                accentColor: const Color(0xFFFF4D4D),
                dotColor: const Color(0xFFFF4D4D),
                selectedFriend: controller.akaFighterFriend,
                onSelectFriend: () => _openFriendsBottomSheet(context, controller, isAka: true),
              ),
              const SizedBox(height: 16),

              // AO (Blue Belt / Corner)
              KarateKaCard(
                cornerTitle: 'AO (Blue Belt)',
                nameController: controller.aoFighterController,
                accentColor: const Color(0xFF4D96FF),
                dotColor: const Color(0xFF4D96FF),
                selectedFriend: controller.aoFighterFriend,
                onSelectFriend: () => _openFriendsBottomSheet(context, controller, isAka: false),
              ),
              const SizedBox(height: 32),

              // 3. Bout Duration Stepper Card
              _buildLargeStepperCard(
                context,
                title: 'BOUT DURATION',
                mainText: 'Duration of Kumite Bout',
                valueStream: controller.boutDurationMinutes,
                onDecrement: controller.decrementBoutDuration,
                onIncrement: controller.incrementBoutDuration,
                suffix: ' MIN',
              ),
  ...
  Widget _buildCategorySelector(BuildContext context, KarateController controller) {
    return Container(
              ...
              children: [
                Expanded(
                  child: _buildCategoryChip(
                    context,
                    label: 'SENIOR (3 MINS)',
                    isSelected: controller.category.value == 'SENIOR_3MIN',
                    onTap: () => controller.setCategory('SENIOR_3MIN'),
                  ),
                ),
                ...
                Expanded(
                  child: _buildCategoryChip(
                    context,
                    label: 'JUNIOR (2 MINS)',
                    isSelected: controller.category.value == 'JUNIOR_2MIN',
                    onTap: () => controller.setCategory('JUNIOR_2MIN'),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildStartMatchButton(BuildContext context, KarateController controller) {
    return Container(
      padding: EdgeInsets.all(ResponsiveHelper.w(20)),
  ...
  void _openFriendsBottomSheet(BuildContext context, KarateController controller, {required bool isAka}) {
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(
                    'Select Karateka (${isAka ? "AKA Red" : "AO Blue"})',
                    style: AppTypography.headlineSm.copyWith(
                      color: AppColors.textPrimary,
                      fontSize: ResponsiveHelper.sp(20),
                      fontWeight: FontWeight.bold,
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (c, idx) {
                      ...
                      return ListTile(
                        ...
                        onTap: () {
                          if (isAka) {
                            controller.akaFighterFriend.value = friend;
                            controller.akaFighterController.text = friend.fullName.isNotEmpty ? friend.fullName : friend.email;
                          } else {
                            controller.aoFighterFriend.value = friend;
                            controller.aoFighterController.text = friend.fullName.isNotEmpty ? friend.fullName : friend.email;
                          }
                          Navigator.pop(ctx);
                        },
                      );
