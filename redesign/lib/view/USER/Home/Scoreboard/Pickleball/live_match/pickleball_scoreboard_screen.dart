...
import 'package:redesign/view/USER/Home/Scoreboard/Pickleball/live_match/widgets/pickleball_score_display.dart';
import 'package:redesign/view/USER/Home/Scoreboard/Pickleball/live_match/widgets/pickleball_action_buttons.dart';

class PickleballScoreboardScreen extends StatefulWidget {
  const PickleballScoreboardScreen({super.key});

  @override
  State<PickleballScoreboardScreen> createState() => _PickleballScoreboardScreenState();
}

class _PickleballScoreboardScreenState extends State<PickleballScoreboardScreen> {
  final PickleballController controller = Get.find<PickleballController>();
  Worker? _stateWorker;
  ...
  void initState() {
    ...
    _stateWorker = ever(controller.liveState, (state) {
      ...
      if (state.isMatchFinished) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            _showGameCompletedDialog(context, controller).then((_) {
              _isDialogShowing = false;
            });
          }
        });
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: Obx(() {
          ...
          return const PickleballActionButtons();
        }),
      ),
    );
  }
  ...
  Future<bool?> _showExitDialog(BuildContext context) {
    return showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(
          'Exit Scoreboard?',
          style: AppTypography.headlineMd.copyWith(color: AppColors.textPrimary).responsive(context),
        ),
        content: Text(
          'Your match progress is saved automatically. You can resume it anytime from the Scoreboards tab.',
          style: AppTypography.bodySm.copyWith(color: AppColors.textSecondary, fontSize: 13).responsive(context),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: Text('Cancel', style: AppTypography.bodySm.copyWith(color: AppColors.mutedText).responsive(context)),
          ),
          ElevatedButton(
  ...
  Future<void> _showGameCompletedDialog(BuildContext context, PickleballController controller) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Game ${state.currentGameIndex + 1} Completed',
                style: AppTypography.headlineMd.copyWith(
                  color: AppColors.textPrimary,
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
        ...
        content: Column(
          ...
          children: [
            Text(
              'Game ${state.currentGameIndex + 1} completed! Tap below to begin Game ${state.currentGameIndex + 2} or undo the last point.',
              style: AppTypography.bodySm.copyWith(color: AppColors.textSecondary, fontSize: 13).responsive(context),
            ),
            ...
            if (!controller.isReadOnly.value && controller.engine.canUndo)
              Center(
                child: TextButton.icon(
                  onPressed: () {
                    Navigator.pop(ctx);
                    controller.undoGameCompletionPoint();
                  },
                  icon: const Icon(Icons.undo, color: AppColors.warning, size: 18),
                  label: Text(
                    'Undo Last Point & Continue Game',
                    style: AppTypography.bodySm.copyWith(
                      color: AppColors.warning,
                      fontWeight: FontWeight.bold,
                    ).responsive(context),
        ...
        actions: [
          ElevatedButton(
            ...
            child: Text(
              'Start Next Game ${state.currentGameIndex + 2}',
              style: AppTypography.bodySm.copyWith(
                color: AppColors.background,
                fontWeight: FontWeight.bold,
              ).responsive(context),
  ...
  Future<void> _showMatchFinishedDialog(BuildContext context, PickleballController controller) async {
    int timeLeft = 60;
