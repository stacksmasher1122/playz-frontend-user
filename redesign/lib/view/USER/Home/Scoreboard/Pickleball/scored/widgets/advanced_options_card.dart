...
class AdvancedOptionsCard extends StatelessWidget {
  ...
  AdvancedOptionsCard({super.key, required this.controller});
      ...
      child: Column(
        ...
        children: [
          ...
          Obx(
            () => _buildSection(
              "Match Type",
              controller.matchTypeOptions,
              controller.selectedMatchType.value,
              controller.changeMatchType,
            ),
          ),
          SizedBox(height: 16),
          Obx(
            () => _buildSection(
              "Court Surface",
              controller.surfaceOptions,
              controller.selectedSurface.value,
              controller.changeCourtSurface,
            ),
          ),
          SizedBox(height: 16),
          Obx(
            () => _buildSection(
              "Skill Level",
              controller.skillLevelOptions,
              controller.selectedSkillLevel.value,
              controller.changeSkillLevel,
            ),
          ),
          SizedBox(height: 16),
          Obx(
            () => _buildSection(
              "Serving Rule",
              controller.servingRuleOptions,
              controller.selectedServingRule.value,
              controller.changeServingRule,
            ),
          ),
          SizedBox(height: 16),
          Row(
          ...
          Obx(
            () => _buildSection(
              "Maximum Score",
              ["11", "15", "21"],
              controller.maximumScore.value.toString(),
              (v) => controller.selectMaxScore(int.parse(v)),
            ),
          ),
          SizedBox(height: 16),
          Obx(
          ...
          Obx(
            () => _buildSection(
              "Court Side",
              ["Random", "Manual"],
              controller.courtSideSelection.value,
              controller.selectCourtSide,
            ),
          ),
        ],
      ),
