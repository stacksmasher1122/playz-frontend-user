...
import 'widgets/tt_scoreboard_header.dart';
import 'widgets/tt_scoring_dock.dart';
import 'widgets/tt_more_actions_sheet.dart';

class TableTennisScoreboardScreen extends StatefulWidget {
  const TableTennisScoreboardScreen({super.key});

  @override
  State<TableTennisScoreboardScreen> createState() =>
      _TableTennisScoreboardScreenState();
}

class _TableTennisScoreboardScreenState
    extends State<TableTennisScoreboardScreen> {
  final TableTennisController controller = Get.find<TableTennisController>();
  Worker? _stateWorker;
  Worker? _undoTimerWorker;
  bool _isMatchEndFlowActive = false;
  bool _isGameBreakFlowActive = false;
  ...
  void initState() {
    ...
    _undoTimerWorker = ever(controller.gamePointUndoSeconds, (seconds) {
      ...
      if (seconds > 0 &&
          !_isGameBreakFlowActive &&
          !_isMatchEndFlowActive &&
          status != 'COMPLETED' &&
          status != 'RETIRED' &&
          mounted) {
        _triggerGameCompletedFlow();
      }
    });
  ...
  void _triggerGameCompletedFlow() {
    _isGameBreakFlowActive = true;
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (ctx, setState) {
            ...
            String gameSummaryText = 'Game Completed!';
            if (state != null && state.gameScores.isNotEmpty) {
              final lastCompletedGameIdx = state.currentGameIndex > 0 ? state.currentGameIndex - 1 : 0;
              if (lastCompletedGameIdx < state.gameScores.length) {
                final gameObj = state.gameScores[lastCompletedGameIdx];
                final winnerName = gameObj.winnerSide == 'A'
                    ? state.matchConfig.homeTeamName
                    : state.matchConfig.awayTeamName;
                gameSummaryText = '$winnerName won Game ${gameObj.gameNumber} (${gameObj.sideAPoints}-${gameObj.sideBPoints})';
              }
            }
            ...
            IconButton(
              ...
              onPressed: () {
                ...
                showModalBottomSheet(
                  ...
                  builder: (_) => TtMoreActionsSheet(
                    homeTeamName: state.matchConfig.homeTeamName,
                    awayTeamName: state.matchConfig.awayTeamName,
                    sideATimeoutsLeft: state.sideATimeoutsLeft,
                    sideBTimeoutsLeft: state.sideBTimeoutsLeft,
                    isExpediteActive: state.isExpediteActive,
                    onCallTimeout: controller.callTimeout,
                    onToggleExpedite: controller.toggleExpedite,
              ...
              child: Column(
                children: [
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          TtScoreboardHeader(state: state),
                  ...
                  TtScoringDock(
                    ...
                    onRecordServiceFault: controller.recordServiceFault,
