...
import 'package:redesign/view/USER/Home/Scoreboard/Judo/widgets/judo_ka_card.dart';

class JudoSetupScreen extends StatelessWidget {
  const JudoSetupScreen({super.key});

  @override
  Widget build(BuildContext context) {
    ResponsiveHelper.init(context);
    final controller = Get.put(JudoController());
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              // 1. Category Selector (Senior 4 Min vs Cadet 3 Min)
              _buildCategorySelector(context, controller),
              const SizedBox(height: 24),

              // 2. COMPETITOR CORNERS Section Header
              Text(
                'JUDOKA CORNERS',
              ...
              // White Corner
              JudoKaCard(
                cornerTitle: 'WHITE CORNER (WHITE GI)',
                nameController: controller.whiteFighterController,
                accentColor: const Color(0xFFE0E0E0),
                dotColor: Colors.white,
                selectedFriend: controller.whiteFighterFriend,
                onSelectFriend: () => _openFriendsBottomSheet(context, controller, isWhite: true),
              ),
              const SizedBox(height: 16),

              // Blue Corner
              JudoKaCard(
                cornerTitle: 'BLUE CORNER (BLUE GI)',
                nameController: controller.blueFighterController,
                accentColor: const Color(0xFF4D96FF),
                dotColor: const Color(0xFF4D96FF),
                selectedFriend: controller.blueFighterFriend,
                onSelectFriend: () => _openFriendsBottomSheet(context, controller, isWhite: false),
              ),
              const SizedBox(height: 32),

              // 3. Contest Duration Stepper Card
              _buildLargeStepperCard(
                context,
                title: 'CONTEST DURATION',
                mainText: 'Duration of IJF Contest',
                valueStream: controller.contestDurationMinutes,
                onDecrement: controller.decrementContestDuration,
                onIncrement: controller.incrementContestDuration,
                suffix: ' MIN',
              ),
  ...
  Widget _buildCategorySelector(BuildContext context, JudoController controller) {
    return Container(
              ...
              children: [
                Expanded(
                  child: _buildCategoryChip(
                    context,
                    label: 'SENIOR (4 MINS)',
                    isSelected: controller.category.value == 'SENIOR_4MIN',
                    onTap: () => controller.setCategory('SENIOR_4MIN'),
                  ),
                ),
                ...
                Expanded(
                  child: _buildCategoryChip(
                    context,
                    label: 'CADET (3 MINS)',
                    isSelected: controller.category.value == 'CADET_3MIN',
                    onTap: () => controller.setCategory('CADET_3MIN'),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildStartMatchButton(BuildContext context, JudoController controller) {
    return Container(
      padding: EdgeInsets.all(ResponsiveHelper.w(20)),
  ...
  void _openFriendsBottomSheet(BuildContext context, JudoController controller, {required bool isWhite}) {
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(
                    'Select Judoka (${isWhite ? "White Corner" : "Blue Corner"})',
                    style: AppTypography.headlineSm.copyWith(
                      color: AppColors.textPrimary,
                      fontSize: ResponsiveHelper.sp(20),
                      fontWeight: FontWeight.bold,
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (c, idx) {
                      ...
                      return ListTile(
                        ...
                        onTap: () {
                          if (isWhite) {
                            controller.whiteFighterFriend.value = friend;
                            controller.whiteFighterController.text = friend.fullName.isNotEmpty ? friend.fullName : friend.email;
                          } else {
                            controller.blueFighterFriend.value = friend;
                            controller.blueFighterController.text = friend.fullName.isNotEmpty ? friend.fullName : friend.email;
                          }
                          Navigator.pop(ctx);
                        },
                      );
