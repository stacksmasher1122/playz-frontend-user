...
import 'package:redesign/view/USER/Home/Scoreboard/Boxing/live_match/widgets/boxing_score_display.dart';
import 'package:redesign/view/USER/Home/Scoreboard/Boxing/live_match/widgets/boxing_action_buttons.dart';

class BoxingScoreboardScreen extends StatefulWidget {
  const BoxingScoreboardScreen({super.key});

  @override
  State<BoxingScoreboardScreen> createState() => _BoxingScoreboardScreenState();
}

class _BoxingScoreboardScreenState extends State<BoxingScoreboardScreen> {
  final BoxingController controller = Get.find<BoxingController>();
  Worker? _stateWorker;
  ...
  void initState() {
    ...
    _stateWorker = ever(controller.liveState, (state) {
      ...
      } else if (state.isRestPeriod) {
        _isDialogShowing = true;
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            _showRestPeriodDialog(context, controller).then((_) {
              _isDialogShowing = false;
            });
          }
        });
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: Obx(() {
          ...
          return const BoxingActionButtons();
        }),
      ),
    );
  }
  ...
  Future<void> _showRestPeriodDialog(BuildContext context, BoxingController controller) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Round ${state.currentRoundIndex + 1} Rest Period',
                style: AppTypography.headlineMd.copyWith(
                  color: AppColors.textPrimary,
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
        ...
        content: Column(
          ...
          children: [
            Text(
              'Round ${state.currentRoundIndex + 1} score submitted. 60 seconds rest break in progress.',
              style: AppTypography.bodySm.copyWith(color: AppColors.textSecondary, fontSize: 13).responsive(context),
            ),
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: () {
              Navigator.pop(ctx);
              controller.advanceNextRound();
            },
            child: Text(
              'Start Round ${state.currentRoundIndex + 2}',
              style: AppTypography.bodySm.copyWith(
                color: AppColors.background,
                fontWeight: FontWeight.bold,
              ).responsive(context),
  ...
  Future<void> _showMatchFinishedDialog(BuildContext context, BoxingController controller) async {
    int timeLeft = 60;
