...
import 'package:redesign/view/USER/Home/Scoreboard/Wrestling/widgets/wrestling_wrestler_card.dart';

class WrestlingSetupScreen extends StatelessWidget {
  const WrestlingSetupScreen({super.key});

  @override
  Widget build(BuildContext context) {
    ResponsiveHelper.init(context);
    final controller = Get.put(WrestlingController());
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              // 1. Style Selection (Freestyle vs Greco-Roman)
              _buildStyleSelector(context, controller),
              const SizedBox(height: 24),

              // 2. WRESTLER CORNERS Section Header
              Text(
                'WRESTLER CORNERS',
              ...
              WrestlingWrestlerCard(
                cornerTitle: 'Red Corner (Wrestler A)',
                nameController: controller.wrestlerAController,
                accentColor: const Color(0xFFFF4D4D),
                dotColor: const Color(0xFFFF4D4D),
                selectedFriend: controller.wrestlerAFriend,
                onSelectFriend: () => _openFriendsBottomSheet(context, controller, isWrestlerA: true),
              ),
              const SizedBox(height: 16),

              // Blue Corner (Wrestler B)
              WrestlingWrestlerCard(
                cornerTitle: 'Blue Corner (Wrestler B)',
                nameController: controller.wrestlerBController,
                accentColor: const Color(0xFF4D96FF),
                dotColor: const Color(0xFF4D96FF),
                selectedFriend: controller.wrestlerBFriend,
                onSelectFriend: () => _openFriendsBottomSheet(context, controller, isWrestlerA: false),
              ),
              const SizedBox(height: 32),

              // 3. Scheduled Periods Stepper Card
              _buildLargeStepperCard(
                context,
                title: 'SCHEDULED PERIODS',
                mainText: 'Periods in Match',
                valueStream: controller.totalPeriods,
                onDecrement: controller.decrementTotalPeriods,
                onIncrement: controller.incrementTotalPeriods,
              ),
              const SizedBox(height: 16),

              // 4. Period Duration Stepper Card
              _buildLargeStepperCard(
                context,
                title: 'PERIOD DURATION',
                mainText: 'Duration of Each Period',
                valueStream: controller.periodDurationMinutes,
                onDecrement: controller.decrementPeriodDuration,
                onIncrement: controller.incrementPeriodDuration,
                suffix: ' MIN',
              ),
  ...
  Widget _buildStyleSelector(BuildContext context, WrestlingController controller) {
    return Container(
              ...
              children: [
                Expanded(
                  child: _buildStyleChip(
                    context,
                    label: 'FREESTYLE (10 PT TF)',
                    isSelected: controller.style.value == 'FREESTYLE',
                    onTap: () => controller.setStyle('FREESTYLE'),
                  ),
                ),
                ...
                Expanded(
                  child: _buildStyleChip(
                    context,
                    label: 'GRECO-ROMAN (8 PT TF)',
                    isSelected: controller.style.value == 'GRECO_ROMAN',
                    onTap: () => controller.setStyle('GRECO_ROMAN'),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildStartMatchButton(BuildContext context, WrestlingController controller) {
    return Container(
      padding: EdgeInsets.all(ResponsiveHelper.w(20)),
  ...
  void _openFriendsBottomSheet(BuildContext context, WrestlingController controller, {required bool isWrestlerA}) {
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(
                    'Select Wrestler (${isWrestlerA ? "Red Corner" : "Blue Corner"})',
                    style: AppTypography.headlineSm.copyWith(
                      color: AppColors.textPrimary,
                      fontSize: ResponsiveHelper.sp(20),
                      fontWeight: FontWeight.bold,
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (c, idx) {
                      ...
                      return ListTile(
                        ...
                        onTap: () {
                          if (isWrestlerA) {
                            controller.wrestlerAFriend.value = friend;
                            controller.wrestlerAController.text = friend.fullName.isNotEmpty ? friend.fullName : friend.email;
                          } else {
                            controller.wrestlerBFriend.value = friend;
                            controller.wrestlerBController.text = friend.fullName.isNotEmpty ? friend.fullName : friend.email;
                          }
                          Navigator.pop(ctx);
                        },
                      );
