...
import 'widgets/squash_score_display.dart';
import 'widgets/squash_game_history_bar.dart';
import 'widgets/squash_action_buttons.dart';

class SquashScoreboardScreen extends StatefulWidget {
  const SquashScoreboardScreen({super.key});

  @override
  State<SquashScoreboardScreen> createState() => _SquashScoreboardScreenState();
}

class _SquashScoreboardScreenState extends State<SquashScoreboardScreen> {
  final SquashController controller = Get.find<SquashController>();
  Worker? _stateWorker;
  ...
  void initState() {
    ...
    _stateWorker = ever(controller.liveState, (state) {
      if (state != null &&
          (state.isGameFinished || state.isMatchFinished) &&
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: Obx(() {
            ...
            return Column(
              children: [
                // 1. TOP PINNED HIGH-VISIBILITY SCOREBOARD SECTION (Always Pinned At Top)
                Padding(
                  padding: EdgeInsets.symmetric(
                    horizontal: ResponsiveHelper.w(16),
                    vertical: ResponsiveHelper.h(8),
                  ...
                  child: Column(
                    children: [
                      SquashGameHistoryBar(state: state),
                      SizedBox(height: ResponsiveHelper.h(10)),
                      SquashScoreDisplay(
                        state: state,
                        onScoreRally: controller.scoreRally,
                        onSelectServeBox: controller.selectServeBox,
                        onSwitchServeBox: controller.switchServeBox,
                        onSelectHihoDeuceOption: controller.selectHihoDeuceOption,
                      ),
                    ],
                  ),
                ),

                // 2. MIDDLE OPTIONAL SCROLLABLE SECTION (For Completed Match Banner or Details)
                Expanded(
        ...
        // 3. FIXED BOTTOM SCORING CONSOLE (Compact & Non-Obscuring)
        bottomNavigationBar: SafeArea(
          child: Obx(() {
            ...
            return Container(
              ...
              child: SquashActionButtons(
                state: state,
                canUndo: controller.engine.canUndo,
                onScoreRally: controller.scoreRally,
                onRecordStroke: controller.recordStroke,
                onRecordNoLet: controller.recordNoLet,
                onRecordConductWarning: controller.recordConductWarning,
                onRecordConductStroke: controller.recordConductStroke,
                onRecordConductGame: controller.recordConductGame,
                onRecordLet: controller.recordLet,
                onSwitchServeBox: controller.switchServeBox,
                onUndo: controller.undoLastAction,
                onStartNextGame: controller.startNextGame,
              ),
            );
          }),
        ),
  ...
  String _getGameWinnerText(SquashMatchState state) {
    if (state.gameHistory.isNotEmpty) {
      final lastGame = state.gameHistory.last;
      final winnerSide = lastGame.winner;
      final winnerName = winnerSide == PlayerSide.sideA
          ? (state.teamA.isNotEmpty ? state.teamA.first.name : 'Side A')
          : (state.teamB.isNotEmpty ? state.teamB.first.name : 'Side B');
      return '$winnerName Won Game ${lastGame.gameNumber} (${lastGame.sideAScore} - ${lastGame.sideBScore})';
    }
    return 'Game Completed!';
  }

  String _getWinnerText(SquashMatchState? state) {
    if (state == null) return 'MATCH COMPLETED';
    final winner = state.matchWinner;
    final team = winner == PlayerSide.sideA ? state.teamA : state.teamB;
    if (team.isNotEmpty) {
      final name = team.first.name;
      return '$name WON (${state.sideAGamesWon} - ${state.sideBGamesWon})';
    }
    return winner == PlayerSide.sideA ? 'Side A WON' : 'Side B WON';
  }
}
