...
class LiveMatchesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: kBg,
        title: Text('Live Matches', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
        centerTitle: true,
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
             return Center(child: Text('Error: ${snapshot.error}', style: TextStyle(color: kRed)));
          }
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return Center(child: Text("No live matches found.", style: TextStyle(color: kMuted)));
          }
            ...
            children: [
              Container(
                ...
                child: Row(
                  children: [
                    Container(
                      width: ResponsiveHelper.w(6), height: ResponsiveHelper.h(6),
                      decoration: BoxDecoration(color: kRed, shape: BoxShape.circle),
                    ),
                    SizedBox(width: 6),
                    Text('LIVE', style: TextStyle(color: kRed, fontSize: ResponsiveHelper.sp(10), fontWeight: FontWeight.w900, letterSpacing: 0.5)),
                  ],
                ),
              ),
              Text(
                dateFormat.format(match.createdAt),
                style: TextStyle(color: kMuted, fontSize: ResponsiveHelper.sp(11), fontWeight: FontWeight.w500),
              ),
            ],
          ),
          SizedBox(height: 20),
            ...
            children: [
              Expanded(
                child: ElevatedButton(
                  ...
                  child: Text('Join Scoring', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14)),
                ),
              ),
            ],
          )
      ...
      children: [
        Row(
          children: [
            ...
            if (isBatting) ...[
              SizedBox(width: 8),
              Icon(Icons.sports_cricket, color: kGreen, size: 16),
            ]
          ],
        ),
        Row(
          ...
          children: [
            ...
            Text(
              '/$wickets',
              style: TextStyle(color: kMuted, fontSize: ResponsiveHelper.sp(12), fontWeight: FontWeight.w600),
            ),
            SizedBox(width: 8),
            Text(
              '($overs.$balls)',
